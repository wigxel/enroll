...
import { ArrowLeft, User, Mail, Calendar, CheckCircle2, Clock, XCircle, FileText } from "lucide-react";
...
export default function StudentDetailsPage() {
    ...
    const enrollmentsResult = safeArray((useQuery(api.enrollments.listByUserId, { userId: studentId }) as any)?.data);
    ...
    const handleCohortChange = async (enrollmentId: Id<"enrollments">, cohortId: string) => {
    ...
    const cohorts = cohortsResultRaw?.data?.cohorts ?? [];
    const student = studentResult?.success ? studentResult.data : null;
    const enrollments = enrollmentsResult;

    if (studentResult === undefined || enrollmentsResult === undefined || cohortsResultRaw === undefined) {
        return (
    ...
    if (student === null) {
        return (
            <div className="py-12 text-center">
                ...
                <p className="mt-2 text-gray-500">The student record you're looking for doesn't exist.</p>
                <button
                    onClick={() => router.back()}
                ...
                <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
                    ...
                    <div className="space-y-6 lg:col-span-1">
                        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
                            <div className="bg-gray-50 px-6 py-8 text-center">
                                ...
                                <h1 className="mt-4 text-xl font-bold text-gray-900">{student.name}</h1>
                                <p className="text-sm font-medium text-primary uppercase tracking-wider">{student.roleName}</p>
                            </div>
                    ...
                    <div className="lg:col-span-2 space-y-6">
                        <div className="flex items-center justify-between">
                            <h2 className="text-xl font-semibold text-gray-900">Enrollment History</h2>
                        ...
                        {enrollments.length === 0 ? (
                            <div className="rounded-xl border border-dashed border-gray-300 bg-white p-12 text-center">
                                ...
                                <h3 className="mt-4 text-sm font-medium text-gray-900">No enrollments found</h3>
                                <p className="mt-2 text-sm text-gray-500">This student hasn't started any enrollment processes yet.</p>
                            </div>
                        ) : (
                                    ...
                                    >
                                        <div className="border-b border-gray-50 bg-gray-50/50 px-6 py-4">
                                            <div className="flex items-center justify-between">
                                                <div>
                                                    ...
                                                    <div className="mt-1 flex items-center gap-2">
                                                        <select
                                                            ...
                                                            onChange={(e) => handleCohortChange(enrollment._id, e.target.value)}
                                                ...
                                                <div className={`flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${enrollment.status === "completed"
                                        ...
                                        <div className="p-6">
                                            <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
                                                <div className="flex flex-col space-y-2">
                                                    <span className="text-xs font-medium text-gray-500 uppercase">Tuition Fee</span>
                                                ...
                                                <div className="flex flex-col space-y-2">
                                                    <span className="text-xs font-medium text-gray-500 uppercase">Orientation Quiz</span>
                                                    <div className="flex items-center text-sm font-medium">
                                                        ...
                                                        {enrollment.steps.quizPassed ? "Passed" : "Not Attempted"}
                                                    </div>
                                                </div>
                                                <div className="flex flex-col space-y-2">
                                                    ...
                                                    <div className="flex items-center text-sm font-medium">
                                                        ...
                                                        {enrollment.steps.documentsSigned ? "Signed" : "Pending"}
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            {enrollment.completedAt && (
                                                <div className="mt-6 border-t border-gray-50 pt-4 flex items-center text-xs text-gray-500">
                                                    ...
                                                    Enrolled on {new Date(enrollment.completedAt).toLocaleDateString()}
                                                </div>
                                            )}
                                        </div>
                                    </div>
